...
# 유전 여부에 따른 탈모 분석
df_hereditary = df.groupby(['is_hereditary', 'bald_prob']).size().reset_index(name='count')

fig, ax = plt.subplots(figsize=(8,6))
barchart = sns.barplot(data=df_hereditary, x='is_hereditary', y='count', hue='bald_prob', order=['Yes', 'No'], ax=ax)
